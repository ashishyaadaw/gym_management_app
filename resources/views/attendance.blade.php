    ...
    <div class="d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-3 mb-4">
        <div>
            ...
            <p class="text-secondary small mb-0">
                {{ $isStaff ? 'Check members in at the front desk and watch the floor traffic' : 'Your visit history' }}
            </p>
        </div>
        @if ($isStaff)
            <input type="date" id="att-date" class="form-control" style="max-width:11rem" max="{{ now()->toDateString() }}" value="{{ now()->toDateString() }}" aria-label="Day">
        @endif
    </div>
...
@push('scripts')
    <script src="{{ asset('js/pages/attendance.js') }}?v={{ filemtime(public_path('js/pages/attendance.js')) }}"></script>
@endpush
